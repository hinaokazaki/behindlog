...
const Button: React.FC<ButtonProps> = ({
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={clsx(
        `inline-flex items-center justify-center rounded-xl border-2 px-4 py-2 text-sm font-semibold transition-colors duration-200 sm:px-5 sm:py-2 sm:text-form-text`,
        styleSet.base,
        !disabled && styleSet.hover,
        disabled &&
          "cursor-not-allowed opacity-60 hover:border-inherit hover:bg-inherit hover:text-inherit",
        className,
      )}
    >
      {children}
